...
BEGIN
    ...
    OPEN CONSUMOS_ID_CURSOR FOR
        ...
        WHERE (P_MES IS NOT NULL AND P_ANO IS NULL AND EXTRACT(YEAR FROM LCC.DATA_REGISTO) = EXTRACT(YEAR FROM
                                                                                                     SYSDATE) AND
               EXTRACT(MONTH FROM LCC.DATA_REGISTO) = P_MES)
           OR (P_MES IS NOT NULL AND P_ANO IS NOT NULL AND EXTRACT(YEAR FROM LCC.DATA_REGISTO) = P_ANO AND
               EXTRACT(MONTH FROM LCC.DATA_REGISTO) = P_MES)
    ...
    -- PREENCHER A TABELA COM OS BONUS PRETENDIDOS, O DEFAULT E 0
    LOOP
...
BEGIN
    ...
    PRCATUALIZARBONUSCAMAREIRAS(12, 2020);
END;
